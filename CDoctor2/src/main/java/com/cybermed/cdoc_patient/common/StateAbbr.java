...
public enum StateAbbr {
     ...
     */
    private static final Map<String, StateAbbr> STATES_BY_ABBR = new HashMap<String, StateAbbr>();

    /* static initializer */
    static {
        for (StateAbbr state : values()) {
     ...
     */
    StateAbbr(String name, String abbreviation) {
        this.name = name;
    ...
    public static StateAbbr valueOfAbbreviation(final String abbr) {
        final StateAbbr state = STATES_BY_ABBR.get(abbr);
        if (state != null) {
            return state;
        } else {
            return UNKNOWN;
    ...
    public static StateAbbr valueOfName(final String name) {
        final String enumName = name.toUpperCase().replaceAll(" ", "_");
        try {
            return valueOf(enumName);
        } catch (final IllegalArgumentException e) {
            return StateAbbr.UNKNOWN;
        }
    }

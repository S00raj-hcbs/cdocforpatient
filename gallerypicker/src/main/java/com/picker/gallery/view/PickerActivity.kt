...
import com.picker.gallery.databinding.ActivityPickerBinding
...
class PickerActivity : AppCompatActivity() {
    ...
    private val PERMISSIONS_CAMERA = 124
    internal var IMAGES_THRESHOLD = 0
    var VIDEOS_THRESHOLD = 0
    internal var REQUEST_RESULT_CODE = 101
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setupTabIcons()
        binding.tabs.getTabAt(0)?.setIcon(selectedTabIcons[0])
    ...
    private fun createImageFile(): File {
        val timeStamp = SimpleDateFormat("yyyyMMdd_HHmmss").format(Date())
        val imageFileName = "JPEG_" + timeStamp + "_"
        val storageDir = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.FROYO) {
    ...
    private fun galleryAddPic() {
        ...
        val out = FileOutputStream(file)
        val bitmap = MediaStore.Images.Media.getBitmap(contentResolver, contentUri)
        val ei = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.ECLAIR) {
            ExifInterface(mCurrentPhotoPath)
        } else {
        ...
        try {
            ...
            ((binding.viewpager.adapter as ViewPagerAdapter).mFragmentList[0] as? PhotosFragment)?.initViews()
        } catch (e: Exception) {
            e.printStackTrace()
        }
    }
    ...
    private val tabIconList: ArrayList<Int> = ArrayList()
    private val tabIcons = intArrayOf(R.drawable.ic_picker_photos_unselected,
        R.drawable.ic_video_unselected)
    ...
    private fun setupTabIcons() {
        tabIconList.add(tabIcons[0])
        tabIconList.add(tabIcons[1])
        binding.tabs.getTabAt(0)?.setIcon(tabIconList[0])
        binding.tabs.getTabAt(1)?.setIcon(tabIconList[1])

...
) : RecyclerView.Adapter<ImageGridAdapter.MyViewHolder>(), FastScrollRecyclerView.SectionedAdapter {
    ...
    private fun handleImageSelection(position: Int) {
        if (threshold != 0) {
            when {
                ...
                imageList[position].isSelected -> {
                    imageList[position].isSelected = false
                }
                getSelectedCount() < threshold -> {  // Fix: Use "<" instead of "<="
                    imageList[position].isSelected = true
                }
                else -> {
